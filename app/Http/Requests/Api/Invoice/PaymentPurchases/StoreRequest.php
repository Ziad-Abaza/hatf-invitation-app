...
namespace App\Http\Requests\Api\Invoice\PaymentPurchases;
...
class StoreRequest extends BaseFormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'element_data.*.invoic_id'     => ['required', Rule::in(auth('owner')->user()->purchases->pluck('id')->toArray())],
            'element_data.*.invoic_amount' => 'nullable|numeric',
            'element_data.*.invoic_all'    => ['required',Rule::in(0,1)],
            'invoice_purchase_id'          =>['nullable', Rule::in(auth('owner')->user()->purchases->pluck('id')->toArray())],
